	...
	public static void main(String[] args) {
		...
		while (true){
			...
			try {
				...
				System.out.println("Please write only integers");
			}
			if(choice == 1) {
				CarsRepositoryFile repo = new CarsRepositoryFile();
				StartMainFile(repo);
			}
			else if (choice == 2) {
				CarsRepositorySerialization repo = new CarsRepositorySerialization();
				StartMainByte(repo);
			}
			else if (choice == -1) {
